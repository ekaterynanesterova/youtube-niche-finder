...
// Trending — единственный источник каналов, не зависящий от наших же
// запросов. Разведка по своим темам возвращает то, что мы уже знаем; чарт
// приносит чужое.
//
// Категорий было две из пятнадцати: «Образование» и «Наука и техника».
// Эмбиент, природа, путешествия и вообще всё, что не подписано наукой, в
// базу не попадало никогда. Каналы со звуками дождя и океана, найденные
// вручную, не нашлись ни один из четырёх — потому что искать их было негде.
//
// Теперь семь: к прежним добавлены «Люди и блоги» (там живёт эмбиент),
// «Музыка» (звуки для сна), «Животные», «Путешествия» и «Развлечения».
// Стоит это один юнит за пару «рынок + категория», то есть четырнадцать
// юнитов из девяти тысяч.
