...
<dt>Набирает ролик у канала без аудитории</dt>
        <dd>Первое число на карточке и самое честное, какое тут можно дать: <b>коридор</b>, а не точка. Нижняя граница — четверть роликов набирает меньше, верхняя — четверть набирает больше. Между ними лежит ровно половина роликов ниши.
        <br><br>Считается по роликам возрастом от недели до двух месяцев на каналах моложе года с чистым стартом. Рядом стоит, по скольким роликам посчитано — ниже двадцати пяти ниша вообще не ранжируется и уходит в «Ещё изучаем».
        <br><br>Раньше верхней границей была девятая десятая, и это была ошибка: на выборке в два десятка роликов она равна второму-третьему значению сверху, то есть почти чистому шуму. При делении выборки пополам она расходилась на 14%, а нынешняя граница — на 9%. Из-за этого «apex predators documentary» стоял третьим в рейтинге на девятнадцати роликах.
        <br><br>Потолок никуда не делся — он показан отдельно и конкретно, живым примером в строке «Лучшее, что сделал новичок с нуля», и там на него можно посмотреть глазами.
        <br><br>По этому же числу строится порядок ниш. Это важно: до сентября сортировка шла по одному показателю, а на карточке стоял другой, и они расходились впятеро.</dd>
        ...
        <dt>Держат цель по набранным просмотрам</dt>
        <dd>Сколько каналов ниши <b>зарабатывают сейчас</b>, и у скольких из них темп не падает. Считается по разнице между дневными срезами: сколько просмотров канал реально набрал за две недели, а не сколько у него накоплено. Разница принципиальная: канал, чей ролик собрал миллион полгода назад и с тех пор молчит, по накопленному выглядит богатым, а зарабатывает почти ноль. Из 159 каналов, которых старая оценка называла вышедшими на цель, живой прирост подтвердил 121, а 24 оказались затухшими.</dd>
        ...
        <dt>Тема и формат — разные вещи</dt>
        <dd>Ниша названа <b>темой</b>: «древний Египет», «глубокое море». Как её искали, написано отдельно, мелким шрифтом. Раньше это было склеено, и каждая ниша называлась «X documentary» — отчего казалось, что на YouTube ничего, кроме документалок, не бывает.<br><br>Слово в запросе при этом нужно, и это проверено замером: по голому «dinosaur» приезжают мультики и игрушки, по «black hole» — детские обучалки, по «disaster» — «Maddie's Nail Disaster! Learn Why Clean Nails Matter». Фильтр по длительности этого не ловит, мультсборники тоже длинные.<br><br>Но одного слова мало. По «village life» без него приезжает почти то же самое, а по «ancient egypt» без него нашёлся ролик MrBeast на 263 млн просмотров, которого со словом не видно вовсе. Поэтому форматов несколько, и какой подходит теме — выясняется замером: инструмент пробует их по очереди и оставляет тот, что приводит длинные ролики от каналов без накопленной аудитории.</dd>
...
function drawVerdict() {
  ...
  const money = (r) => [
    [r.rangeLo == null ? '—' : num(r.rangeLo) + ' – ' + num(r.rangeHi),
     'набирает ролик у канала без аудитории · по ' + plural(r.rangeN, 'ролику', 'роликам', 'роликам')],
  ...
  host.innerHTML =
    ...
    V.slice(0, 6).map((r, i) =>
      ...
      '<h4>' + (r.subject || r.query) + (r.ru ? ' <em>' + r.ru + '</em>' : '') + '</h4>' +
      '<div class="mk">' + r.market + ' рынок · ' + r.group
        + (r.format ? ' · ищем как «' + r.query + '»' : ' · ищем голым запросом')
        + '</div>' +
