  ...
  <footer>
    <details class="gloss">
      ...
      <dl>
        ...
        <dt>Живут ли старые ролики</dt>
        <dd>Работает ли на тебя видео, снятое полгода назад. Больше единицы — старое продолжает приносить просмотры, место в нише можно занять. Меньше — беговая дорожка: перестала выпускать, доход исчез. Считается по живому приросту между срезами, поэтому первые дни цифра шумит.</dd>
...
function drawVerdict() {
  ...
  const money = (r) => [
    ...
    [r.matureUsd ? '$' + num(r.matureUsd) : '—', 'в месяц у выросших'],
    ...
    [r.shelf == null ? '—' : '×' + r.shelf.toFixed(1), 'живут ли старые ролики'],
    ...
    V.slice(0, 6).map((r, i) =>
      ...
      (r.shelf != null
        ? '<p><span class="lab">долговечность:</span> ролики старше полугода собирают в '
          ...
          + (r.shelfLive
              ? ' — посчитано по живому приросту за ' + plural(P.gainWindowDays || 1, 'день', 'дня', 'дней')
                + ((P.gainWindowDays || 1) < 7 ? ' наблюдения, окно ещё короткое.' : ' наблюдения.')
              : ' — оценка по накопленным просмотрам, точная появится через неделю сбора.') + '</p>'
        : '') +
      (r.catalogMax
