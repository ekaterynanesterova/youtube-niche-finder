...
export function score(n) {
  if (n.permeability == null || !n.outlierChannels) return null;
...
export function renderReport(m, seeds) {
  ...
  L.push('## Рейтинг');
  L.push('');
  L.push('| # | Ниша | Проницаемость | Выбросы (каналов) | Медиана выброса | Мусорность | Лайки | Публикаций/нед | Доверие |');
  L.push('|---|------|---------------|-------------------|-----------------|------------|-------|----------------|---------|');
  withData.forEach((n, i) => {
    L.push(`| ${i + 1} | **${n.id}** <br><sub>${n.group}</sub> | ${pct(n.permeability)} | ${n.outlierChannels} | ${num(n.medianOutlierViews)} | ${pct(n.slopShare)} | ${pct(n.medianLikeRate)} | ${one(n.medianUploadsPerWeek)} | ${n.confidence} |`);
  });
  L.push('');

  L.push('## Немецкий против английского');
  L.push('');
  L.push('Один и тот же сюжет в двух рынках. Смотрим, где дверь шире.');
  L.push('');
  ...
  for (const n of withData) {
    ...
    L.push(`| ${n.id} | ${pct(d.permeability)} (${d.outlierChannels}) | ${num(d.medianViews)} | ${pct(e.permeability)} (${e.outlierChannels}) | ${num(e.medianViews)} |`);
  }
  L.push('');
  ...
  if (noData.length) {
    ...
    L.push('');
    L.push('Каналов или выбросов слишком мало для выводов. Нужны прогоны.');
    L.push('');
  }
  ...
  L.push('- **Выбросы (каналов)** — сколько *разных* каналов дали выброс. Один канал считается один раз, иначе метрику ломает единственный везунчик.');
